...
Player:GetMouse().KeyDown:Connect(function(Key)
    ...
    if getgenv().Macro.Settings.IsEnabled then
        -- Check if the pressed key matches the defined Macro key
        if Key == tostring(getgenv().KeyTrigger.Macro):lower() then
...
if getgenv().AntiLock.Settings.Enable then
    ...
    getgenv().key = tostring(getgenv().KeyTrigger.AntiLock):lower()  -- Use KeyTrigger for AntiLock
...
llocal UserInputService = game:GetService("UserInputService")
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then  -- Ignore input if it's being processed by a GUI
        if input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.Spin] then  -- Update to use the correct key reference
            performSpin()
        end
    end
end)
...
local function onKeyPress(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.Rejoin] and not gameProcessed then
...
UserInputService.InputBegan:Connect(function(input)
    ...
    if getgenv().PanicSystem.Settings.IsActive then
        ...
        if input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.PanicSystem] then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.TriggerBot] then
        handleShootingMode()
    end
end)

UserInputService.InputEnded:Connect(function(input, gameProcessed)
    if input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.TriggerBot] then
...
RunService.RenderStepped:Connect(function()
    if getgenv().TriggerBot.Settings.Mode == "hold" and UserInputService:IsKeyDown(Enum.KeyCode[getgenv().KeyTrigger.TriggerBot]) then
        TriggerBotAction()
    end
end)
